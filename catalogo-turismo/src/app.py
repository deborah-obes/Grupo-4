...
def abrir_album(url: str) -> None:
    """Abrir un álbum de fotos en el navegador por defecto."""
    webbrowser.open(url)


def abrir_maps(lugar: str, categoria: str) -> None:
    """Abrir una búsqueda en Google Maps según lugar y categoría."""
...
for i, album in enumerate(albumes):
    ...
    marco = tk.Frame(
        frame_albumes, bg="#FFFFFF", bd=2, relief="ridge", padx=10, pady=10
    )
...
autor = tk.Label(
    ventana,
    text="Realizado por: Grupo 4 (Deborah Obes, Brenda Torres, Argüello Luis Mathias)",
